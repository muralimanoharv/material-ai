    ...
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                return OAuthErrorResponse(status_code=400, detail='Upstream OAuth Error')
                ...
                _logger.error(f'ERROR: Unexpected HTTP error from {url}: {e}', exc_info=e)
                return OAuthErrorResponse(status_code=500, detail='Internal Server Error')
            except JSONDecodeError as e:
                _logger.error(f'ERROR: Error decoding JSON response from {url}: {e}', exc_info=e)
                return OAuthErrorResponse(status_code=401, detail='Response token JSON decode error')
            except KeyError as e:
                _logger.error(f'ERROR: Missing properties in response from {url}: {e}', exc_info=e)
                return OAuthErrorResponse(status_code=401, detail='Response token missing access_token')
